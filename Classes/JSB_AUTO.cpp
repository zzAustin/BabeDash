...
JSBool js_functionTest(JSContext* cx, uint32_t argc, jsval* vp)
{
	...
	AustinJSB::AustinJSBinding* cobj = NULL;
	obj = JS_THIS_OBJECT(cx,vp);
	js_proxy_t* proxy = jsb_get_js_proxy(obj);
	cobj = (AustinJSB::AustinJSBinding*)(proxy?proxy->ptr:NULL);
...
JSBool js_constructor(JSContext* cx, uint32_t argc, jsval* vp)
{
	...
	if(argc == 0)
	{
		AustinJSB::AustinJSBinding* cobj = new AustinJSB::AustinJSBinding();
		//well, it doesn't have to be an ccobject
		...
		TypeTest<AustinJSB::AustinJSBinding> t;
...
JSBool js_create(JSContext* cx, uint32_t argc, jsval* vp)
{
	...
	if(argc == 0)
	{
		AustinJSB::AustinJSBinding* ret = AustinJSB::AustinJSBinding::create();
		jsval jsret;
		do 
		{
			if(ret){
				js_proxy_t* proxy = js_get_or_create_proxy<AustinJSB::AustinJSBinding>(cx,ret);
...
void js_register(JSContext* cx,JSObject* global)
{
	...
	jsb_class->name = "AustinJSBinding";
	...
	JSBool found;
	JS_SetPropertyAttributes(cx,global,"AustinJSB",JSPROP_ENUMERATE|JSPROP_READONLY,&found);

	TypeTest<AustinJSB::AustinJSBinding> t;
	js_type_class_t* p;
	uint32_t typeId = t.s_id();
		CCLOG("ABOUT TO HASH_FIND_INT");
	HASH_FIND_INT(_js_global_type_ht, &typeId, p);
	CCLOG("ABOUT TO HASH_FIND_INT D");

    if(!p){
	    CCLOG("ABOUT TO HASH_FIND_INT AGAIN");
		...
		p->parentProto = NULL;
		  CCLOG("ABOUT TO HASH_FIND_INT AGAIN!!!! D");
...
void register_all(JSContext* cx,JSObject* obj){
	jsval nsval;
	JSObject* ns;
	JS_GetProperty(cx,obj,"AustinJSB",&nsval);

	if(nsval == JSVAL_VOID){
		ns = JS_NewObject(cx,NULL,NULL,NULL);
		nsval = OBJECT_TO_JSVAL(ns);
		JS_SetProperty(cx,obj,"AustinJSB",&nsval);
	}
	else{
